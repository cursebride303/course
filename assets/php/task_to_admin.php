...
move_uploaded_file($image_path_tmp, "$image_path/$image_name"); //Помещаем картинку по указанному пути 
        ...
        header("Location:../../suggest_task.php");
        ...
        if(!empty($text) && empty($image_name)){
            ...
            $suggest_task = $pdo->prepare("INSERT INTO requests(user_id ,title, subject_id, class_id, text, answer) VALUES ('$user_id', '$title', '$subject', '$class', '$text', '$answer')");
            $suggest_task->execute();
            $_SESSION['suggest_task_message'] = "Задача отправлена на модерацию!";
            header("Location:../../suggest_task.php");
            }
            ...
            $suggest_task = $pdo->prepare("INSERT INTO requests(user_id ,title, subject_id, class_id, text, additional_text, answer) VALUES ('$user_id', '$title', '$subject', '$class', '$img', '$additional_text', '$answer')");
            $suggest_task->execute();
            $_SESSION['suggest_task_message'] = "Задача отправлена на модерацию!";
            header("Location:../../suggest_task.php");
        }else{
            $suggest_task = $pdo->prepare("INSERT INTO requests(user_id ,title, subject_id, class_id, text, answer) VALUES ('$user_id', '$title', '$subject', '$class', '$img', '$answer')");
            $suggest_task->execute();
            $_SESSION['suggest_task_message'] = "Задача отправлена на модерацию!";
            header("Location:../../suggest_task.php");
        }
